...
module.exports = (io) => {
    ...
    io.on('connection', (socket) => {
            ...
            socket.on('decline-activity', (params) => {
                if (params && socket) {
                    logger.info(`Socket: ${socket.keyForFirebase} Decline ${params.activity.activity_name}`);
                    if (params.to.id in allUsers) {
            ...
            socket.on('approve-activity', (params) => {
                if (params && socket) {
                    logger.info(`Socket: ${socket.keyForFirebase} approve ${params.activity.activity_name} to ${params.to.fullName}`);
